import { RouterCollection } from "./RouterCollection.js";
...
export class StateHandler {
    ...
    #validVerb = ["GET", "POST"]; // Allowed verbs
    ...
    constructor(configs = {}) {
        ...
        if(!(router instanceof RouterCollection)) {
            throw new Error("The first argument is expectign router ");
        }
         */
    ...
    postTo(path, request = {}) {
        ...
        if(!(request instanceof FormData)) {
            formData = this.toFormData(request);
        }
    ...
    pushState(path, state = {}) {
        ...
        if(!this.#validVerb.includes(state.method)) {
            throw new Error('The verb (http method) "'+state.method+'" is not allowed. Supported verbs: '+this.#validVerb.join(", "));
        }
    ...
    map(verb, pattern, controller) {
        ...
        // Make sure every verb is upper case
    ...
    dispatcher(fn) {
        ...
        inst.state(function(updatedRoute) {
            ...
            const dispatcher = inst.validateDispatch(inst.#state.method, updatedRoute.hash);
     ...
     * @return {object|false}
     */
    validateDispatch(method, dipatch) {
        const inst = this;
        ...
        for(let i = 0; i < router.length; i++) {
            ...
            if(router[i].verb.includes(method)) for(let x = 0; x < uri.length; x++) {
